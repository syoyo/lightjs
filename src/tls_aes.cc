#include "tls.h"

namespace lightjs {
namespace tls {

// AES S-Box
const uint8_t AES128::SBOX[256] = {
  
};

// AES Inverse S-Box
const uint8_t AES128::INV_SBOX[256] = {
  
};

// AES Round Constants
const uint8_t AES128::RCON[11] = {
  0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36
};

// Helper functions for AES
static inline uint32_t subWord(uint32_t w, const uint8_t* sbox) {
  return (static_cast<uint32_t>(sbox[(w >> 24) & 0xff]) << 24) |
         (static_cast<uint32_t>(sbox[(w >> 16) & 0xff]) << 16) |
         (static_cast<uint32_t>(sbox[(w >> 8) & 0xff]) << 8) |
         static_cast<uint32_t>(sbox[w & 0xff]);
}

static inline uint32_t rotWord(uint32_t w) {
  return (w << 8) | (w >> 24);
}

static inline uint8_t xtime(uint8_t x) {
  return (x << 1) ^ ((x >> 7) * 0x1b);
}

static inline uint8_t multiply(uint8_t x, uint8_t y) {
  uint8_t result = 0;
  uint8_t temp = x;
  for (int i = 0; i < 8; i++) {
    if (y & 1) result ^= temp;
    temp = xtime(temp);
    y >>= 1;
  }
  return result;
}

AES128::AES128() {
  std::memset(roundKeys_, 0, sizeof(roundKeys_));
}

AES128::AES128(const uint8_t* key) {
  setKey(key);
}

void AES128::setKey(const uint8_t* key) {
  keyExpansion(key);
}

void AES128::keyExpansion(const uint8_t* key) {
  // First 4 words are the key itself
  for (int i = 0; i < 4; i++) {
    roundKeys_[i] = (static_cast<uint32_t>(key[4*i]) << 24) |
                    (static_cast<uint32_t>(key[4*i+1]) << 16) |
                    (static_cast<uint32_t>(key[4*i+2]) << 8) |
                    static_cast<uint32_t>(key[4*i+3]);
  }

  // Generate remaining round keys
  for (int i = 4; i < 44; i++) {
    uint32_t temp = roundKeys_[i-1];
    if (i % 4 == 0) {
      temp = subWord(rotWord(temp), SBOX) ^ (static_cast<uint32_t>(RCON[i/4]) << 24);
    }
    roundKeys_[i] = roundKeys_[i-4] ^ temp;
  }
}

void AES128::encryptBlock(const uint8_t* in, uint8_t* out) const {
  uint8_t state[16];
  std::memcpy(state, in, 16);

  // AddRoundKey for round 0
  for (int i = 0; i < 4; i++) {
    state[4*i] ^= (roundKeys_[i] >> 24) & 0xff;
    state[4*i+1] ^= (roundKeys_[i] >> 16) & 0xff;
    state[4*i+2] ^= (roundKeys_[i] >> 8) & 0xff;
    state[4*i+3] ^= roundKeys_[i] & 0xff;
  }

  // Rounds 1-9
  for (int round = 1; round < NUM_ROUNDS; round++) {
    // SubBytes
    for (int i = 0; i < 16; i++) {
      state[i] = SBOX[state[i]];
    }

    // ShiftRows
    uint8_t temp;
    temp = state[1]; state[1] = state[5]; state[5] = state[9]; state[9] = state[13]; state[13] = temp;
    temp = state[2]; state[2] = state[10]; state[10] = temp;
    temp = state[6]; state[6] = state[14]; state[14] = temp;
    temp = state[3]; state[3] = state[15]; state[15] = state[11]; state[11] = state[7]; state[7] = temp;

    // MixColumns
    for (int i = 0; i < 4; i++) {
      uint8_t a = state[4*i];
      uint8_t b = state[4*i+1];
      uint8_t c = state[4*i+2];
      uint8_t d = state[4*i+3];
      state[4*i] = xtime(a) ^ xtime(b) ^ b ^ c ^ d;
      state[4*i+1] = a ^ xtime(b) ^ xtime(c) ^ c ^ d;
      state[4*i+2] = a ^ b ^ xtime(c) ^ xtime(d) ^ d;
      state[4*i+3] = xtime(a) ^ a ^ b ^ c ^ xtime(d);
    }

    // AddRoundKey
    for (int i = 0; i < 4; i++) {
      state[4*i] ^= (roundKeys_[4*round+i] >> 24) & 0xff;
      state[4*i+1] ^= (roundKeys_[4*round+i] >> 16) & 0xff;
      state[4*i+2] ^= (roundKeys_[4*round+i] >> 8) & 0xff;
      state[4*i+3] ^= roundKeys_[4*round+i] & 0xff;
    }
  }

  // Final round (no MixColumns)
  for (int i = 0; i < 16; i++) {
    state[i] = SBOX[state[i]];
  }

  uint8_t temp;
  temp = state[1]; state[1] = state[5]; state[5] = state[9]; state[9] = state[13]; state[13] = temp;
  temp = state[2]; state[2] = state[10]; state[10] = temp;
  temp = state[6]; state[6] = state[14]; state[14] = temp;
  temp = state[3]; state[3] = state[15]; state[15] = state[11]; state[11] = state[7]; state[7] = temp;

  for (int i = 0; i < 4; i++) {
    state[4*i] ^= (roundKeys_[40+i] >> 24) & 0xff;
    state[4*i+1] ^= (roundKeys_[40+i] >> 16) & 0xff;
    state[4*i+2] ^= (roundKeys_[40+i] >> 8) & 0xff;
    state[4*i+3] ^= roundKeys_[40+i] & 0xff;
  }

  std::memcpy(out, state, 16);
}

void AES128::decryptBlock(const uint8_t* in, uint8_t* out) const {
  uint8_t state[16];
  std::memcpy(state, in, 16);

  // AddRoundKey for final round
  for (int i = 0; i < 4; i++) {
    state[4*i] ^= (roundKeys_[40+i] >> 24) & 0xff;
    state[4*i+1] ^= (roundKeys_[40+i] >> 16) & 0xff;
    state[4*i+2] ^= (roundKeys_[40+i] >> 8) & 0xff;
    state[4*i+3] ^= roundKeys_[40+i] & 0xff;
  }

  // Rounds 9-1
  for (int round = NUM_ROUNDS - 1; round > 0; round--) {
    // InvShiftRows
    uint8_t temp;
    temp = state[13]; state[13] = state[9]; state[9] = state[5]; state[5] = state[1]; state[1] = temp;
    temp = state[2]; state[2] = state[10]; state[10] = temp;
    temp = state[6]; state[6] = state[14]; state[14] = temp;
    temp = state[7]; state[7] = state[11]; state[11] = state[15]; state[15] = state[3]; state[3] = temp;

    // InvSubBytes
    for (int i = 0; i < 16; i++) {
      state[i] = INV_SBOX[state[i]];
    }

    // AddRoundKey
    for (int i = 0; i < 4; i++) {
      state[4*i] ^= (roundKeys_[4*round+i] >> 24) & 0xff;
      state[4*i+1] ^= (roundKeys_[4*round+i] >> 16) & 0xff;
      state[4*i+2] ^= (roundKeys_[4*round+i] >> 8) & 0xff;
      state[4*i+3] ^= roundKeys_[4*round+i] & 0xff;
    }

    // InvMixColumns
    for (int i = 0; i < 4; i++) {
      uint8_t a = state[4*i];
      uint8_t b = state[4*i+1];
      uint8_t c = state[4*i+2];
      uint8_t d = state[4*i+3];
      state[4*i] = multiply(a, 0x0e) ^ multiply(b, 0x0b) ^ multiply(c, 0x0d) ^ multiply(d, 0x09);
      state[4*i+1] = multiply(a, 0x09) ^ multiply(b, 0x0e) ^ multiply(c, 0x0b) ^ multiply(d, 0x0d);
      state[4*i+2] = multiply(a, 0x0d) ^ multiply(b, 0x09) ^ multiply(c, 0x0e) ^ multiply(d, 0x0b);
      state[4*i+3] = multiply(a, 0x0b) ^ multiply(b, 0x0d) ^ multiply(c, 0x09) ^ multiply(d, 0x0e);
    }
  }

  // Final round
  uint8_t temp;
  temp = state[13]; state[13] = state[9]; state[9] = state[5]; state[5] = state[1]; state[1] = temp;
  temp = state[2]; state[2] = state[10]; state[10] = temp;
  temp = state[6]; state[6] = state[14]; state[14] = temp;
  temp = state[7]; state[7] = state[11]; state[11] = state[15]; state[15] = state[3]; state[3] = temp;

  for (int i = 0; i < 16; i++) {
    state[i] = INV_SBOX[state[i]];
  }

  for (int i = 0; i < 4; i++) {
    state[4*i] ^= (roundKeys_[i] >> 24) & 0xff;
    state[4*i+1] ^= (roundKeys_[i] >> 16) & 0xff;
    state[4*i+2] ^= (roundKeys_[i] >> 8) & 0xff;
    state[4*i+3] ^= roundKeys_[i] & 0xff;
  }

  std::memcpy(out, state, 16);
}

// AES-256 implementation
AES256::AES256() {
  std::memset(roundKeys_, 0, sizeof(roundKeys_));
}

AES256::AES256(const uint8_t* key) {
  setKey(key);
}

void AES256::setKey(const uint8_t* key) {
  keyExpansion(key);
}

void AES256::keyExpansion(const uint8_t* key) {
  // First 8 words are the key itself
  for (int i = 0; i < 8; i++) {
    roundKeys_[i] = (static_cast<uint32_t>(key[4*i]) << 24) |
                    (static_cast<uint32_t>(key[4*i+1]) << 16) |
                    (static_cast<uint32_t>(key[4*i+2]) << 8) |
                    static_cast<uint32_t>(key[4*i+3]);
  }

  // Generate remaining round keys
  for (int i = 8; i < 60; i++) {
    uint32_t temp = roundKeys_[i-1];
    if (i % 8 == 0) {
      temp = subWord(rotWord(temp), AES128::SBOX) ^ (static_cast<uint32_t>(AES128::RCON[i/8]) << 24);
    } else if (i % 8 == 4) {
      temp = subWord(temp, AES128::SBOX);
    }
    roundKeys_[i] = roundKeys_[i-8] ^ temp;
  }
}

void AES256::encryptBlock(const uint8_t* in, uint8_t* out) const {
  uint8_t state[16];
  std::memcpy(state, in, 16);

  // AddRoundKey for round 0
  for (int i = 0; i < 4; i++) {
    state[4*i] ^= (roundKeys_[i] >> 24) & 0xff;
    state[4*i+1] ^= (roundKeys_[i] >> 16) & 0xff;
    state[4*i+2] ^= (roundKeys_[i] >> 8) & 0xff;
    state[4*i+3] ^= roundKeys_[i] & 0xff;
  }

  // Rounds 1-13
  for (int round = 1; round < NUM_ROUNDS; round++) {
    // SubBytes
    for (int i = 0; i < 16; i++) {
      state[i] = AES128::SBOX[state[i]];
    }

    // ShiftRows
    uint8_t temp;
    temp = state[1]; state[1] = state[5]; state[5] = state[9]; state[9] = state[13]; state[13] = temp;
    temp = state[2]; state[2] = state[10]; state[10] = temp;
    temp = state[6]; state[6] = state[14]; state[14] = temp;
    temp = state[3]; state[3] = state[15]; state[15] = state[11]; state[11] = state[7]; state[7] = temp;

    // MixColumns
    for (int i = 0; i < 4; i++) {
      uint8_t a = state[4*i];
      uint8_t b = state[4*i+1];
      uint8_t c = state[4*i+2];
      uint8_t d = state[4*i+3];
      state[4*i] = xtime(a) ^ xtime(b) ^ b ^ c ^ d;
      state[4*i+1] = a ^ xtime(b) ^ xtime(c) ^ c ^ d;
      state[4*i+2] = a ^ b ^ xtime(c) ^ xtime(d) ^ d;
      state[4*i+3] = xtime(a) ^ a ^ b ^ c ^ xtime(d);
    }

    // AddRoundKey
    for (int i = 0; i < 4; i++) {
      state[4*i] ^= (roundKeys_[4*round+i] >> 24) & 0xff;
      state[4*i+1] ^= (roundKeys_[4*round+i] >> 16) & 0xff;
      state[4*i+2] ^= (roundKeys_[4*round+i] >> 8) & 0xff;
      state[4*i+3] ^= roundKeys_[4*round+i] & 0xff;
    }
  }

  // Final round
  for (int i = 0; i < 16; i++) {
    state[i] = AES128::SBOX[state[i]];
  }

  uint8_t temp;
  temp = state[1]; state[1] = state[5]; state[5] = state[9]; state[9] = state[13]; state[13] = temp;
  temp = state[2]; state[2] = state[10]; state[10] = temp;
  temp = state[6]; state[6] = state[14]; state[14] = temp;
  temp = state[3]; state[3] = state[15]; state[15] = state[11]; state[11] = state[7]; state[7] = temp;

  for (int i = 0; i < 4; i++) {
    state[4*i] ^= (roundKeys_[56+i] >> 24) & 0xff;
    state[4*i+1] ^= (roundKeys_[56+i] >> 16) & 0xff;
    state[4*i+2] ^= (roundKeys_[56+i] >> 8) & 0xff;
    state[4*i+3] ^= roundKeys_[56+i] & 0xff;
  }

  std::memcpy(out, state, 16);
}

void AES256::decryptBlock(const uint8_t* in, uint8_t* out) const {
  uint8_t state[16];
  std::memcpy(state, in, 16);

  // AddRoundKey for final round
  for (int i = 0; i < 4; i++) {
    state[4*i] ^= (roundKeys_[56+i] >> 24) & 0xff;
    state[4*i+1] ^= (roundKeys_[56+i] >> 16) & 0xff;
    state[4*i+2] ^= (roundKeys_[56+i] >> 8) & 0xff;
    state[4*i+3] ^= roundKeys_[56+i] & 0xff;
  }

  // Rounds 13-1
  for (int round = NUM_ROUNDS - 1; round > 0; round--) {
    // InvShiftRows
    uint8_t temp;
    temp = state[13]; state[13] = state[9]; state[9] = state[5]; state[5] = state[1]; state[1] = temp;
    temp = state[2]; state[2] = state[10]; state[10] = temp;
    temp = state[6]; state[6] = state[14]; state[14] = temp;
    temp = state[7]; state[7] = state[11]; state[11] = state[15]; state[15] = state[3]; state[3] = temp;

    // InvSubBytes
    for (int i = 0; i < 16; i++) {
      state[i] = AES128::INV_SBOX[state[i]];
    }

    // AddRoundKey
    for (int i = 0; i < 4; i++) {
      state[4*i] ^= (roundKeys_[4*round+i] >> 24) & 0xff;
      state[4*i+1] ^= (roundKeys_[4*round+i] >> 16) & 0xff;
      state[4*i+2] ^= (roundKeys_[4*round+i] >> 8) & 0xff;
      state[4*i+3] ^= roundKeys_[4*round+i] & 0xff;
    }

    // InvMixColumns
    for (int i = 0; i < 4; i++) {
      uint8_t a = state[4*i];
      uint8_t b = state[4*i+1];
      uint8_t c = state[4*i+2];
      uint8_t d = state[4*i+3];
      state[4*i] = multiply(a, 0x0e) ^ multiply(b, 0x0b) ^ multiply(c, 0x0d) ^ multiply(d, 0x09);
      state[4*i+1] = multiply(a, 0x09) ^ multiply(b, 0x0e) ^ multiply(c, 0x0b) ^ multiply(d, 0x0d);
      state[4*i+2] = multiply(a, 0x0d) ^ multiply(b, 0x09) ^ multiply(c, 0x0e) ^ multiply(d, 0x0b);
      state[4*i+3] = multiply(a, 0x0b) ^ multiply(b, 0x0d) ^ multiply(c, 0x09) ^ multiply(d, 0x0e);
    }
  }

  // Final round
  uint8_t temp;
  temp = state[13]; state[13] = state[9]; state[9] = state[5]; state[5] = state[1]; state[1] = temp;
  temp = state[2]; state[2] = state[10]; state[10] = temp;
  temp = state[6]; state[6] = state[14]; state[14] = temp;
  temp = state[7]; state[7] = state[11]; state[11] = state[15]; state[15] = state[3]; state[3] = temp;

  for (int i = 0; i < 16; i++) {
    state[i] = AES128::INV_SBOX[state[i]];
  }

  for (int i = 0; i < 4; i++) {
    state[4*i] ^= (roundKeys_[i] >> 24) & 0xff;
    state[4*i+1] ^= (roundKeys_[i] >> 16) & 0xff;
    state[4*i+2] ^= (roundKeys_[i] >> 8) & 0xff;
    state[4*i+3] ^= roundKeys_[i] & 0xff;
  }

  std::memcpy(out, state, 16);
}

}  // namespace tls
}  // namespace lightjs
